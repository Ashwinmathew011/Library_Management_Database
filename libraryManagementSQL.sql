...
USE LIB
	...
	CREATE TABLE book (
		...
		CONSTRAINT fk1 FOREIGN KEY REFERENCES publisher(PublisherName) ON UPDATE CASCADE ON DELETE CASCADE,
	)
...
WHERE B.BookID=BC.BookID AND B.Title='The Lost Tribe' AND BC.BranchID=LB.BranchID AND LB.BranchName='Sharpstown'
...
CREATE PROCEDURE MORE_THAN_FIVES
AS
BEGIN
SELECT B.BorrowerName,B.BorrowerAddress, CardNo
FROM borrower B
